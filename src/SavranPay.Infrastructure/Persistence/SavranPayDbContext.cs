...
public sealed class SavranPayDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<UserSessionEntity>(entity =>
        {
            ...
            entity.HasOne(item => item.User).WithMany(item => item.Sessions).HasForeignKey(item => item.UserId);
            entity.HasOne(item => item.RefreshToken).WithOne(item => item.Session).HasForeignKey<UserSessionEntity>(item => item.RefreshTokenId);
        });
